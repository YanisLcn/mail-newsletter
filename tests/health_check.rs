...
fn spawn_app() -> String {
    ...
    let server = mail_newsletter::startup::run(listener).expect("Failed to bun address");
...
async fn suscribe_returns_a_200_for_valid_form_data() {
    ...
    assert_eq!(saved.email, "arnaud_du_bourg@gmail.com");
    assert_eq!(saved.name, "du bourg");
}
